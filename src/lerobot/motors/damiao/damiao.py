...
class DamiaoMotorsBus(MotorsBusBase):
    ...
    def disconnect(self, disable_torque: bool = True) -> None:
        ...
        if disable_torque:
            # Passes with retries: a single failure must not skip later motors
            # (gripper is last), and a late MIT frame can re-enable a motor.
            try:
                self.disable_torque(num_retry=2, raise_on_error=False)
                time.sleep(MEDIUM_TIMEOUT_SEC)
                if "gripper" in self.motors:
                    self.disable_torque(["gripper"], num_retry=2, raise_on_error=False)
                    time.sleep(MEDIUM_TIMEOUT_SEC)
                self.disable_torque(num_retry=1, raise_on_error=False)
                if "gripper" in self.motors:
                    self.disable_torque(["gripper"], num_retry=2, raise_on_error=False)
            except Exception as e:
    ...
    def disable_torque(
        ...
        Continues through the full motor list even if one motor fails, so a
        transient error on an earlier joint cannot leave the gripper enabled.
        """
        target_motors = self._get_motors_list(motors)
        failures: list[str] = []
        for motor in target_motors:
            ...
            if last_error is not None:
                failures.append(f"{motor}: {last_error}")
                logger.warning("Failed to disable torque on %s: %s", motor, last_error)
        if failures and raise_on_error:
            raise RuntimeError("Failed to disable torque on: " + "; ".join(failures))
