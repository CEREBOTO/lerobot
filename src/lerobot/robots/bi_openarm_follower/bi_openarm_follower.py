...
class BiOpenArmFollower(BimanualMixin, Robot):
    ...
    def __init__(self, config: BiOpenArmFollowerConfig):
        ...
        self._send_pool = concurrent.futures.ThreadPoolExecutor(max_workers=2)
    ...
    def send_action(
        ...
        # Issue both arms concurrently so one CAN bus does not wait on the other.
        left_future = self._send_pool.submit(self.left_arm.send_action, left_action, custom_kp, custom_kd)
        right_future = self._send_pool.submit(self.right_arm.send_action, right_action, custom_kp, custom_kd)
    ...
    def _disable_arm_torque(self, arm: OpenArmFollower, arm_name: str) -> None:
        """Best-effort torque disable, with extra gripper frames (gripper is last / easy to miss)."""
        if not arm.is_connected or not arm.bus.is_connected:
            return
        try:
            arm.bus.disable_torque(num_retry=3, raise_on_error=False)
            if "gripper" in arm.bus.motors:
                # Gripper often stays enabled if a late MIT frame wins the race or a
                # single disable is dropped — pulse disable a few times.
                for _ in range(3):
                    arm.bus.disable_torque(["gripper"], num_retry=2, raise_on_error=False)
                    time.sleep(0.015)
        except BaseException as exc:  # noqa: BLE001 — keep going; includes KeyboardInterrupt
            logger.warning("Failed to disable torque on %s arm: %s", arm_name, exc)

    def disconnect(self) -> None:
        # Finish/cancel in-flight MIT commands before disabling torque; otherwise a
        # late send_action can re-enable motors (especially the gripper) after disable.
        try:
            self._send_pool.shutdown(wait=True, cancel_futures=True)
        except BaseException as exc:  # noqa: BLE001
            logger.warning("Send-pool shutdown failed: %s", exc)

        # Disable BOTH arms while buses are still open. Right first: sequential
        # left-then-right disconnect + a second Ctrl+C was leaving the right gripper enabled.
        for arm_name, arm in (("right", self.right_arm), ("left", self.left_arm)):
            self._disable_arm_torque(arm, arm_name)
        ...
        for arm_name, arm in (("right", self.right_arm), ("left", self.left_arm)):
            try:
                ...
                logger.warning("Disconnect failed for %s arm: %s", arm_name, exc)
