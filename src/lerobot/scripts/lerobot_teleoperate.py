...
def teleop_loop(
    ...
    # Observation is only required for visualization / Unitree feedback. Skipping the
    # full dual-arm CAN state read roughly halves bus load and removes a major stutter source.
    need_observation = display_data or robot.name == "unitree_g1"
    while True:
        loop_start = time.perf_counter()

        obs = robot.get_observation() if need_observation else {}
...
def teleoperate(cfg: TeleoperateConfig):
    ...
    try:
        ...
        # A second Ctrl+C during cleanup must not skip follower torque disable.
        if cfg.display_data:
            try:
                shutdown_visualization(cfg.display_mode)
            except BaseException:  # noqa: BLE001
                pass
        for name, closer in (("teleop", teleop.disconnect), ("robot", robot.disconnect)):
            try:
                closer()
            except KeyboardInterrupt:
                logging.warning("KeyboardInterrupt during %s.disconnect(); continuing cleanup", name)
            except Exception:
                logging.exception("Error during %s.disconnect()", name)
